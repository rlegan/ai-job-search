...
- **Job portals:** "This fork targets the **French market**: `wttj-search` (Welcome to the Jungle) and `stationf-search` (STATION F campus startups) are enabled, alongside the country-agnostic `linkedin-search` and `freehire-search`. The four Danish demo portals are installed but disabled. `/scrape` auto-discovers whatever portal skills are installed under `.agents/skills/`. Do these cover your search, or do you use other boards (APEC, HelloWork, Indeed.fr, a sector-specific board)?" If the user needs a board that is not shipped, guide them to `/add-portal` (market-specific skills live in their fork). WebSearch/`site:` queries remain the fallback for portals without a CLI skill.
- **CV language:** "Should your CVs be written in English (widely accepted in Paris tech and at international employers), or in French (expected by most French-speaking employers and for CDI roles outside tech)?" Record the answer as a `CV language: <language>` line in CLAUDE.md's Identity section. Cover letters always match each posting's language automatically; this setting governs the CV only. If the user is unsure, keep English and note they can re-run `/setup --section search` to change it. If they choose French, remember that every `\section{}` heading and the References boilerplate must be translated too (see `05-cv-templates.md`).
