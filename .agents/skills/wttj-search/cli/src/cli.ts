#!/usr/bin/env bun
// Self-contained CLI for Welcome to the Jungle (welcometothejungle.com). No CLI
// framework and zero runtime dependencies, so it runs anywhere `bun` is present
// with nothing installed beyond the repo clone.
//
// ⚠️ Personal use only — see SKILL.md. Search goes to the Algolia host the site's
// own front-end calls (with the site's published search-only key), never to the
// robots-disallowed search URL. Keep volume low; no bulk or commercial use.
...
const HELP = `wttj-cli — search Welcome to the Jungle (welcometothejungle.com)
...
SEARCH FLAGS
  ...
  --location, -l <cities> City/cities, comma = OR. e.g. -l Paris,Lyon
  --region <states>       Region(s) as the site spells them, e.g. --region Ile-de-France
...
DETAIL
  <id>                    "<org-slug>/<job-slug>" from a search result's id, or a
                          full https://www.welcometothejungle.com/fr/companies/.../jobs/... URL
...
EXAMPLES
  ...
  bun run src/cli.ts search -q "développeur" --region Ile-de-France --sort date --format table
  bun run src/cli.ts detail gitguardian/senior-data-engineer_paris_GITGU_1O04qgL --format plain

Personal use only: keep volume low, no bulk or commercial use (see SKILL.md).
Data: the site's own public Algolia index (read-only, no account); ~95,000 live
postings, ~93% of them in France.
`
