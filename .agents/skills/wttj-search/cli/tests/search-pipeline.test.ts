...
// we make of what comes back — including which index a recency query goes to.
// Offline, so CI covers it; the live suite in commands.test.ts (opt-in) covers
// reachability.
...
/** Mock both calls the CLI makes: the runtime-env document, then the Algolia query. */
function mockPortal(
  hits: WkHit[],
  nbHits = hits.length,
): { body: () => Record<string, unknown>; index: () => string } {
  let sent: Record<string, unknown> = {};
  let index = "";
  globalThis.fetch = (async (url: string | URL | Request, init?: RequestInit) => {
    const href = url.toString();
    if (href.includes("/api/env")) {
      return new Response(
        'window.env = {"PUBLIC_ALGOLIA_API_KEY_CLIENT":"KEY","PUBLIC_ALGOLIA_APPLICATION_ID":"APP"}',
        { status: 200, headers: { "content-type": "application/javascript" } },
      );
    }
    index = decodeURIComponent(href.split("/indexes/")[1]?.split("/")[0] ?? "");
...
describe("query construction", () => {
  ...
  test("the window carries dedup headroom above --limit", () => {
    // WTTJ repeats one job across up to 16 website records, so a limit-sized
    // window would return far fewer distinct jobs than asked for.
    expect(windowSize(opts({ limit: 20 }))).toBe(160);
    expect(windowSize(opts({ limit: 1 }))).toBe(40);
    expect(windowSize(opts({ limit: 400 }))).toBe(1000); // Algolia's cap
  });

  test("recency modes switch to the date-sorted replica index", async () => {
    expect(wantsDateOrder(opts())).toBe(false);
    expect(wantsDateOrder(opts({ jobage: 7 }))).toBe(true);
    expect(wantsDateOrder(opts({ sort: "date" }))).toBe(true);

    const plain = mockPortal([hit()]);
    captureStdout();
    await runSearch(opts());
    expect(plain.index()).toBe("wk_cms_jobs_production");

    const recent = mockPortal([hit()]);
    captureStdout();
    await runSearch(opts({ jobage: 7 }));
    expect(recent.index()).toBe("wk_cms_jobs_production_published_at_desc");
  });
...
describe("response handling", () => {
  ...
  test("records repeated per syndicated website collapse to one result", async () => {
    mockPortal([hit(), hit(), hit()], 3);
    const out = captureStdout();
    await runSearch(opts());
    const body = JSON.parse(out.get());
    expect(body.results).toHaveLength(1);
    // meta.total stays the raw Algolia count, which overstates distinct jobs.
  ...
  test("an Algolia error exits 1 with a JSON error on stderr, nothing on stdout", async () => {
    globalThis.fetch = (async (url: string | URL | Request) => {
      const href = url.toString();
      if (href.includes("/api/env")) {
        return new Response('window.env = {"PUBLIC_ALGOLIA_API_KEY_CLIENT":"KEY"}', { status: 200 });
      }
      return new Response(JSON.stringify({ message: "Method not allowed with this referer" }), {
        status: 403,
        headers: { "content-type": "application/json" },
      });
    }) as typeof fetch;
    ...
    expect(err.error).toMatch(/Method not allowed with this referer/);
  });
